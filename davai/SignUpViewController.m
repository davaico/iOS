...
- (void)viewDidLoad {
    ...
    self.emailTextField.layer.borderColor = [[UIColor colorWithRed:237.0f/255.0f green:237.0f/255.0f blue:237.0f/255.0f alpha:1.0f]CGColor];
    ...
    self.emailClearButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.emailClearButton setImage:[UIImage imageNamed:@"Delete Button"] forState:UIControlStateNormal];
    [self.emailClearButton setFrame:CGRectMake(0.0f, 0.0f, 15.0f, 15.0f)];
    self.emailTextField.rightView = self.emailClearButton;
    self.emailTextField.rightViewMode = UITextFieldViewModeWhileEditing;
    [self.emailClearButton addTarget:self action:@selector(clearText:) forControlEvents:UIControlEventTouchUpInside];
    self.emailClearButton.imageEdgeInsets = UIEdgeInsetsMake(0, -5, 0, 5);
...
- (void)clearText:(id)sender
{
    ...
    if (sender == self.emailClearButton)
